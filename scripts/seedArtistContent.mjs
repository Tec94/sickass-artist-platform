...
try {
  const result = await client.mutation(api.artistContent.uploadArtistContentPayload, {
    payload,
    source: '/data/artist-scraped-data.json',
    token: uploadToken,
  });
  console.log('Uploaded artist content payload to Convex.', result);
} catch (error) {
  console.error(
    'Failed to upload artist content payload to Convex:',

              ...
              <div key={`${item.variantId}-${idx}`} className="flex gap-4 group">
                ...
                <div className="flex-1 flex flex-col">
                  <div className="flex justify-between items-start gap-2">
                    <h3 className="text-white font-bold uppercase text-xs leading-tight tracking-wide">{item.productName}</h3>
                    <span className="text-zinc-100 font-bold text-sm shrink-0">${(item.currentPrice / 100).toFixed(2)}</span>
                  </div>
